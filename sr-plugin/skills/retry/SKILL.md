...
Resume a failed `/sr:implement` run for this project (read name from CLAUDE.md or package.json). Reads pipeline state written by the implement pipeline to identify which phases completed and which failed, then re-executes only the remaining phases.
