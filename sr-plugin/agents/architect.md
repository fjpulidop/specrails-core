...
- Group tasks by layer when appropriate: the project layers defined in CLAUDE.md
- Tag each task with its layer: the layer labels defined in CLAUDE.md

### 4. Respect the Architecture

This project follows this architecture:
```
Read the architecture from CLAUDE.md — look for an "Architecture" section or diagram
```

Read layer conventions from `.claude/rules/` if present

- Always check scoped context: any scoped CLAUDE.md files in subdirectories and `.claude/rules/`
- Always check `.claude/rules/` for conditional conventions per layer

### 5. Key Warnings to Always Consider
the Warnings section of CLAUDE.md
...
2. **Compare against the current surface** by reading:
   ...
   - `templates/**/*.md` for `{{PLACEHOLDER}}` keys in template files
