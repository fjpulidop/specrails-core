...
| **Ledger** | Tablas de Core en `run.sqlite` con la verdad de intentos, invocaciones, receipts, uso y presupuesto. El checkpoint de LangGraph guarda el estado del grafo; el ledger, la evidencia. El resultado durable de un nodo (`writes`, pending writes de LangGraph) y su evidencia terminal se escriben en la misma transacción. El snapshot agregado de `checkpoints` se materializa después; la recuperación usa los pending writes para no repetir nodos ya confirmados. |
| **Host** | Desktop: quien lanza, observa, pausa, reanuda, cancela y entrega. |
| **Legado** | El runner actual de Core (`src/agent-runtime/workflow.ts`, `core-host.ts`, `durable-store.ts`, `graph-checkpointer.ts`) y el grafo `specrails-implementation` v7 en código. Se conserva hasta Core 7 para reanudar runs antiguos y para Desktops antiguos. |
...
1. **Compilación**: `compileDefinition(def, deps)` valida schema, reglas 2.2, roles y `paramsSchema` de cada nodo; construye el `StateGraph`: `addNode(id, wrapper(piece), { retryPolicy })`, aristas estáticas para `next` único, `addConditionalEdges(id, router, { label: target | END })` para varias etiquetas, `Send` para `map`, subgrafos para `component`/`implementation`, `interruptBefore` para `gate`; `compile({ checkpointer, store })`; `recursionLimit = maxTransitions + 1` como defensa adicional; un contador global de visitas aplica `maxTransitions` a subgrafos y ramas, porque el límite de LangGraph cuenta supersteps por invocación y no visitas globales.
2. **Envoltorio de nodo**: antes de ejecutar comprueba presupuesto (`$usage` frente a `budget`) ⇒ `blocked` con `budget_exhausted`; comprueba cancelación; resuelve `{{run.*}}`; registra el intento en el ledger (`attempts`); ejecuta la pieza; escribe salida, `$history`, `$usage`; actualiza `$candidate` tras efecto de escritura; pone `$verified = null` si hubo escritura después de un `verify`; incrementa `$transitions`; confirma evidencia terminal y pending writes del nodo en la misma transacción desde la integración del saver; el checkpoint agregado posterior no vuelve a ejecutar el nodo. No se mantiene una transacción SQLite durante la llamada al proveedor.
...
## 12. Decisiones y cuestiones de diseño previas a C3

La evidencia de C1 debe fijar el binding SQLite, las fronteras transaccionales, los límites reales de subgrafos/fan-out y la fuente de eventos antes de implementar C3. No se toma una decisión de binding ni se cambia `engines.node` en la planificación.

- El hash de publicación necesita un procedimiento explícito para calcular `version` antes del primer envío de la definición. El validador deberá distinguir normalización/cálculo de identidad de la comprobación estricta de una identidad suministrada; fijar el protocolo exacto antes de C3/D1.
- `$lastOutcome` y `$item` aparecen en las tareas como estado de compilación pero no son canales públicos de la sección 3: especificar su ámbito y reducción antes de C3/C6.
- C1 confirma localmente la frontera de `putWrites`: sus resultados pendientes y la evidencia de fixture se confirman juntos; el snapshot `put` viene después. Antes de C3, diseñar y probar `attemptId`, visitas, receipts, eventos con sequence e idempotencia bajo ramas concurrentes. La prueba C1 no demuestra el ledger completo de producción.
- `verify` tiene efecto de escritura y crea evidencia: la actualización de `$candidate` debe preceder al sellado de `$verified`; el propio receipt no debe quedar invalidado por el mismo wrapper.
- `signal` es un segundo proceso escritor frente al lease del ejecutor: deberá usar transacciones cortas serializadas por SQLite sin adquirir el lease de ejecución, sin retener una transacción durante llamadas a proveedores. El protocolo exacto se confirma con los spikes.
- Las capacidades `fanOut`, `openRoles` y `steeringInbox` se anuncian solo al completar sus piezas, aunque ejemplos del estado final las muestren juntas.
- La concurrencia por defecto es 1. El límite normativo del schema es 8; la suposición histórica del plan de un máximo 4 queda corregida aquí y en el registro de decisiones.
- La implementación de `shell` usa las utilidades portables de `agent-runtime/cli-process.ts` o una utilidad compartida correctamente extraída; no importa `installer/` desde el motor.
- El catálogo completo contiene 16 piezas (los epígrafes agrupan algunas). Las nueve piezas básicas C4 son `prompt`, `shell`, `openspec-validate`, `openspec-archive`, `condition`, `approval`, `question`, `gate`, `end`.

Estas cuestiones no bloquean C0 ni la preparación de C1/D0; mantienen bloqueada la implementación C3 hasta su resolución, corrección del contrato y evidencia documentada.

### Evidencia local C1 (2026-09-26)

Node 22.22.3, macOS arm64: 200 fronteras confirmadas sobrevivieron a SIGKILL sin repetir nodos, y los cortes antes/durante la transacción no publicaron evidencia terminal. La señal `updates` no garantiza commit (6/6 prematuras con saver retrasado 5 ms); `streamEvents` produjo 6/6 fines de nodo antes de commit. Decisión de adaptación: progreso por writer/custom y ciclo durable por observador del ledger tras commit. Un grafo con siete visitas anidadas terminó con `recursionLimit: 5`, por lo que es obligatorio el contador global. Los informes `docs/engine-v2/spikes/` y los artifacts JSON del runner contienen las medidas; Linux/Windows, ensamblado Desktop y aceptación final siguen pendientes.

### Implementation note (2026-09-26): capability advertisement and frozen request

- `runtime api` now returns `engineVersion: 2`, `nodeKindsVersion` (`NODE_KINDS_VERSION`, exported from `src/agent-runtime/engine/piece-registry.ts`), `nodeKinds: string[]` (the sixteen kinds of the validation registry in registration order: `prompt`, `role-turn`, `decider`, `verify`, `shell`, `openspec-validate`, `openspec-archive`, `condition`, `end`, `approval`, `question`, `gate`, `component`, `map`, `implementation`, `join`; test-only pieces are never registered there) and the additional capabilities `engineV2: 1`, `workflowDefinitions: 1`, `fanOut: 1`, `fork: 1`, `steeringInbox: 1`. `apiVersion` stays `1` and `workflowVersions` stays `['7']`. `steeringInbox` is advertised on the strength of the durable inbox evidence recorded in `CHECKPOINT-C3-C7-C8.md`; the full Desktop steering UI (D7) remains pending.
- `integration-contract.json` keeps `schemaVersion: "5.1"` (additive) and sets `agentRuntime.engine = { version: 2, definitionSchema: 'schemas/workflow-definition.schema.json', nodeKindsVersion }`, `agentRuntime.nodeKinds` to the same sixteen kinds and leaves `builtins` unchanged. New informational strings: `agentRuntime.engineStatePath` (`.../agent-workflow/run.sqlite`) and `agentRuntime.frozenRequest`. `src/agent-runtime/integration-contract.test.ts` enforces parity between the contract, `runtime api`, `runtime workflows list` and `validationPieceRegistry().kinds()`.
- Frozen request (section 9, now implemented): both engines write `agent-runtime-request.json` at `<backlogRoot>/.specrails/pipeline/<runId>/` with `wx` semantics (identical bytes are idempotent; different bytes are `run_exists` on v2 and the existing "Run configuration changed" error on legacy). The legacy run writes `workflow: { id: 'specrails-implementation', version: '7', source: 'builtin', definitionHash: null, engine: 1 }`; a definition run writes `workflow: { id, version, source: 'definition', definitionHash: version, engine: 2 }` before creating `run.sqlite`, so a crash between the two leaves a request that an identical repeated `run` completes. `runtime fork` and `resume --invalidate` materialize the same request for the destination run from its `runs` row.
- Engine selection: `resume` and `status` read `workflow.engine` from the frozen request. `2` routes to the definition engine; `1` or a request without the block routes to the legacy runtime (`resume` then also checks `sameRuntimeIdentity` as before). A definition database without any request file (a fork created before this note) still resolves to the definition engine. `--run-dir` continues to bypass the context.
- `resume` rejects `--definition` and `--config` with `invalid_arguments` on both engines (the legacy path keeps its string message for `--config`/`--change` and uses the structured code for `--definition`).
- `runtime-status` and `runtime-result` from the definition engine carry `workflow: { id, version, source, definitionHash, engine: 2 }`, matching the frozen request block. No other event field changed.

### Paired continuation decision (2026-09-26, Codex)

- Crash injection is external test-preload instrumentation, never a production environment switch. The installed-package matrix runs the actual CLI and production lease TTL.
- `runtime signal` returns `{type:"runtime-signal-accepted",id,acceptedAt}` with Core's stored timestamp. `state.steering` returns `{receipts,pending,consumed,truncated,consumptionReported:true}`. Receipts have `{id,acceptedAt,preview,length,status,consumedAttemptId?,consumedAt?}`; pending first, then newest, maximum 512 and preview maximum 240 characters. Totals include receipts outside the returned window. Desktop persists no competing receipt authority.
- Desktop accepts `{text,requestId}` at POST `/agent-runtime/runs/:runId/steer`, uses the retained CLI and stdin, and reuses ids for retry. MCP exposes `runtime_steer` as write, not a new AI spawn. New receipt acceptance never implies consumption.
