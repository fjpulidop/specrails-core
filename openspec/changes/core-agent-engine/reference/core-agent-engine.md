...
Decisión C2 (2026-09-26): abrir roles sobre C0 sin esperar la persistencia de C1. Se preservan requests built-in antiguos con defaults y configuración congelada sin reescritura. No exponer `descriptor.compact` sin un protocolo genérico probado: roles nuevos ejecutan el bucle libre guardado; mantienen modelos/esfuerzo explícitos, límites, reuse y gates de calidad existentes. Reservar `fixer` a la corrección actual. Kimi expande skills con `--render-only` sin inferencia y ejecuta por el adaptador protegido existente; nunca llamar al runner de ejecución saltándose los permisos. Ver contrato C2 para IDs y errores. `extractPromptInputs` ya tolera Developer summary ausente: conservarlo y cubrirlo por regresión. La release y schema de Desktop siguen emparejados con D1b.

Evidencia C2 (2026-09-26, commit `4ad57b54e34b6f4a7508bbaa9233eb344892bd8a`, Node 22.22.3, macOS ARM64): `npm run ci` completo aprobado: 68 archivos, 1.030 tests aprobados y 1 skip preexistente; statements 86,77%, branches 79,12%, functions 91,36% y lines 92,46%, sin bajar umbrales. `check:package` verificó los dos entrypoints, cuatro ensamblados de proveedor y cuatro diarios congelados. OpenSpec estricto válido; 12 snapshots completos de argv contra C0 preservan los tres roles por cuatro proveedores y los fingerprints workflow 7/instructions 10. La revisión está en PR #387, apilada sobre C0 #385. D1b continúa como gate de publicación, separado de las comprobaciones Core ya completadas.
