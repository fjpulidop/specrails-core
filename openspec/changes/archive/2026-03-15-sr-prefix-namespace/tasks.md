...
- [x] 2.1 Create `templates/commands/specrails/` directory
- [x] 2.2 Move all 8 workflow command files into `templates/commands/specrails/`: implement.md, batch-implement.md, product-backlog.md, update-product-driven-backlog.md, health-check.md, compat-check.md, refactor-recommender.md, why.md
...
- [x] 4.1 Update `batch-implement.md`: all `/implement` refs → `/specrails:implement`, agent name refs → sr-prefixed
- [x] 4.2 Update `product-backlog.md`: `subagent_type: product-analyst` → `sr-product-analyst`, command refs → `/specrails:*`
- [x] 4.3 Update `update-product-driven-backlog.md`: command refs → `/specrails:*`
- [x] 4.4 Update `why.md`: agent name refs (architect, developer, reviewer) → sr-prefixed, `/implement` → `/specrails:implement`
- [x] 4.5 Update `refactor-recommender.md`: self-reference → `/specrails:refactor-recommender`
...
- [x] 5.4 Update command name references (e.g., `/implement` → `/specrails:implement`)
...
- [x] 7.1 Update `openspec/specs/implement.md`: all agent refs → sr-prefixed, command refs → `/specrails:implement`
- [x] 7.2 Update `openspec/specs/batch-implement.md`: `/implement` → `/specrails:implement` references
- [x] 7.3 Update `openspec/specs/confidence-scoring.md`: reviewer → sr-reviewer, developer → sr-developer, architect → sr-architect, `/implement` → `/specrails:implement`
...
- [x] 9.3 Move command files to `.claude/commands/specrails/` directory
- [x] 9.4 Update all internal references in `.claude/commands/specrails/implement.md` (mirror template changes)
- [x] 9.5 Update all internal references in other `.claude/commands/specrails/*.md` files
...
- [x] 11.1 Update `specrails-web/docs/agents.md`: all agent names → sr-prefixed, trigger commands → `/specrails:*`
- [x] 11.2 Update `specrails-web/docs/workflows.md`: all command names → `/specrails:*`, agent names → sr-prefixed, opsx references unchanged
- [x] 11.3 Update `specrails-web/docs/installation.md`: agent file structure → sr-prefixed, command structure → sr/ subdirectory
- [x] 11.4 Update `specrails-web/docs/getting-started.md`: command examples → `/specrails:*`, agent count description
- [x] 11.5 Update `specrails-web/docs/concepts.md`: agent name references → sr-prefixed
- [x] 11.6 Update `specrails-web/docs/customization.md`: command references → `/specrails:*`

## 12. Update specrails-web Components [cli]

- [x] 12.1 Update `specrails-web/src/components/CommandsSection.tsx`: all command names → `/specrails:*`
- [x] 12.2 Update `specrails-web/src/components/DemoSection.tsx`: agent names → sr-prefixed, command names → `/specrails:*`
- [x] 12.3 Update `specrails-web/src/components/FeaturesSection.tsx`: command references → `/specrails:*`
- [x] 12.4 Update `specrails-web/src/components/AgentsSection.tsx`: agent names → sr-prefixed (if referenced)
- [x] 12.5 Update `specrails-web/src/lib/docs-registry.ts`: command name references → `/specrails:*`
...
- [x] 13.3 Move command files to `specrails-web/specrails/.claude/commands/specrails/`
...
- [x] 13.6 Move command templates to `specrails-web/specrails/templates/commands/specrails/`
...
- [x] 14.1 Update `specrails-web/.claude/web-manager/README.md`: command examples → `/specrails:*`
- [x] 14.2 Update `specrails-web/.claude/web-manager/server/spawner.test.ts`: `/implement` → `/specrails:implement` in test cases
...
- [x] 14.6 Update `specrails-web/CLAUDE.md`: command references → `/specrails:*`
