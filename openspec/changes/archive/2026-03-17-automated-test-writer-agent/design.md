...
## Part 1: specrails — `/sr:test` Command Template
...
### File: `.claude/commands/sr/test.md`
...
The manager's `config.ts::scanCommands()` already scans all `.md` files in `.claude/commands/sr/`. When `test.md` is installed into a target repo's `.claude/commands/sr/`, it is automatically discovered, its frontmatter is parsed, and it appears in the dashboard command grid. No changes to `config.ts` are required.
...
The `test` command does NOT need a wizard (no issue picker, no branch selection). It accepts an optional file argument. The UX design is: clicking "Test Writer" opens a `TestWizard` — a small modal that accepts an optional comma-separated file path list. If the user submits with empty input, the command is spawned as `/sr:test` (no arguments, meaning "all changed files"). If the user enters paths, it spawns `/sr:test src/foo.ts,src/bar.ts`.
...
This widget shows the last test-writer job result for the active project. It fetches from the existing `/jobs?limit=1&command=/sr:test` endpoint (the `listJobs` endpoint in `project-router.ts` already supports filtering by command prefix through the `status` query param — but it does NOT support command filtering yet).

**Design decision**: Rather than adding a new API endpoint (which would touch `project-router.ts` and need tests), the widget fetches the last 20 jobs and filters client-side for `command.includes('/sr:test')`. This is pragmatic for v1: the job list is small per project, the filter is trivial, and it avoids server-side changes. If job volume grows, a `?command=` filter can be added later.
...
- Tests that `.claude/commands/sr/test.md` exists
...
These tests use `vi.spyOn(fs, ...)` mocks (same pattern as `config.test.ts`) to simulate a `.claude/commands/sr/test.md` file with the expected frontmatter and verify that `scanCommands` (exported from `config.ts`) returns the correct `CommandInfo` object with phases populated.
...
User clicks "Test Writer" in dashboard
  ...
  → TestWizard calls POST /projects/:id/spawn { command: "/sr:test src/foo.ts" }
  → project-router.ts → QueueManager.enqueue("/sr:test src/foo.ts")
  → QueueManager._resolveCommand reads .claude/commands/sr/test.md
