...
`specrails-core runtime workflows list` emits `{ type: 'runtime-workflows', nodeKindsVersion, nodeKinds: PieceDescriptor[], definitionSchema, builtins }`. The current catalog is `nodeKindsVersion` 1 with 16 kinds (`NODE_KINDS_VERSION` in `src/agent-runtime/engine/cli.ts`). `runtime api` and `integration-contract.json` still advertise `engine.version: 1`, `nodeKindsVersion: 0` and an empty `nodeKinds` list: capability advertisement is a pending integration gate, not a property of the catalog below.
