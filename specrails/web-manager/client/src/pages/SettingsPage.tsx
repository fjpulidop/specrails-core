  ...
  if (isLoading) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-3">
        {[0, 1, 2].map((i) => (
          <div key={i} className="h-32 bg-dracula-current/30 rounded-lg animate-pulse" />
        ))}
      </div>
    )
  }
...
function TrackerStatus({
  ...
  return (
    <div className="flex items-center gap-3 p-3 rounded-md border border-border/30 bg-dracula-current/10">
      <div className="flex items-center gap-2 flex-1">
        {authenticated ? (
          <CheckCircle2 className="w-4 h-4 text-dracula-green" />
        ) : available ? (
          <AlertCircle className="w-4 h-4 text-dracula-orange" />
        ) : (
        ...
        <div>
          ...
          <p className="text-[10px] text-muted-foreground">
            {!available && (
              <>
                Not installed.{' '}
                <a href={setupUrl} target="_blank" rel="noopener noreferrer" className="text-dracula-purple hover:underline">
                  Install CLI
                </a>
              </>
            )}
        ...
        >
          <div
            className={`w-3.5 h-3.5 rounded-full border-2 flex items-center justify-center ${
              isActive ? 'border-dracula-purple' : 'border-border/30'
            }`}
          >
            {isActive && <div className="w-1.5 h-1.5 bg-dracula-purple rounded-full" />}
          </div>
          <span className={isActive ? 'text-dracula-purple' : 'text-muted-foreground'}>
