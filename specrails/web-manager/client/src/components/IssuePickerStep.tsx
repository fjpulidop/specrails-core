  ...
  if (isNoTracker) {
    return (
      <div className="flex flex-col items-center gap-3 py-8 text-center">
        <AlertCircle className="w-8 h-8 text-dracula-orange" />
        <div>
          <p className="text-sm font-medium">No issue tracker configured</p>
          <p className="text-xs text-muted-foreground mt-1">
            Configure GitHub or Jira in{' '}
            <Link to="/settings" className="text-dracula-purple hover:underline">
              Settings
            </Link>{' '}
            to browse issues.
          </p>
      ...
      <div className="max-h-[300px] overflow-y-auto space-y-1 pr-1">
        {isLoading ? (
          <div className="space-y-1">
            {[0, 1, 2, 3].map((i) => (
              <div key={i} className="h-10 bg-dracula-current/30 rounded animate-pulse" />
            ))}
          </div>
        ) : issues.length === 0 ? (
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isSelected
                    ? 'bg-dracula-current/30 border border-dracula-purple/30'
                ...
                <div className="flex items-center gap-1.5 shrink-0 mt-0.5">
                  <div
                    className={cn(
                      ...
                      isSelected ? 'bg-dracula-purple border-dracula-purple' : 'border-border/30'
                    )}
                  >
                    {isSelected && (
...
export function BatchFreeFormStep({ items, onItemsChange }: BatchFreeFormStepProps) {
  ...
  return (
    <div className="space-y-3">
      {items.map((item, idx) => (
        <div key={idx} className="space-y-2 p-3 border border-border/30 rounded-md bg-dracula-current/20">
      ...
      <Button
        ...
        size="sm"
        onClick={addItem}
        className="w-full border-dashed border-border/30"
      >
        + Add another feature
      </Button>
    </div>
