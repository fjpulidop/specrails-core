...
describe('open role contracts', () => {
  ...
  it('uses a declared custom-role escalation only for its existing protocol repair slot', async () => {
    const input = config()
    ...
    expect(selectRoleRoute('security-reviewer', resolveRoleDescriptor(input, 'security-reviewer'), 'initial', { history: [] }).tier).toBe('base')
    expect(selectRoleRoute('security-reviewer', resolveRoleDescriptor(input, 'security-reviewer'), 'repair', { history: [] }).tier).toBe('escalation')
  })
})
