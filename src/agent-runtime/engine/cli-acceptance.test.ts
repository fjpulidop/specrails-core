...
describe.each(Object.keys(entries) as Entry[])('structured fatal errors through %s', entry => {
  ...
  it('rejects resume with a definition and fork under an active lease without touching the run', async () => {
    ...
    expect(readFileSync(database('leased'))).toEqual(bytes)
    const db = await RunDatabase.open(database('leased')), lease = new RunLease(db, 'leased'), token = lease.acquire('another-executor')
    const revision = db.revision, rows = db.transaction(undefined, () => db.rowsAt(revision))
    try {
      ...
      expect(db.transaction(undefined, () => db.rowsAt(db.revision))).toEqual(rows)
    } finally { lease.release(token); db.close() }
    const releasedBytes = readFileSync(database('leased'))
    ...
    expect(readFileSync(database('leased'))).toEqual(releasedBytes)
...
describe('fork and invalidate through the CLI', () => {
  it('forks a historical cut, resumes the fork and leaves the source database byte-identical', async () => {
    ...
    // The cut is before ask executes: its interruption is created on first resume.
    expect(status.last).toMatchObject({ engineVersion: 2, forkOf: 'source', state: { runId: 'copy', status: 'running', lease: null, pendingInterrupts: [] } })
