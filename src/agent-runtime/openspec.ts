...
export class OpenSpecCommandError extends Error {
  constructor(message: string, public readonly exitCode: number | undefined, public readonly stdout: string, public readonly processCode?: string) {
    super(message)
...
export async function runOpenSpec(cli: string, root: string, args: string[], signal?: AbortSignal): Promise<string> {
  try {
    ...
    throw new OpenSpecCommandError(`OpenSpec ${args[0]} failed (${result.status ?? result.code ?? 'process error'}): ${detail}`,
      typeof result.status === 'number' ? result.status : typeof result.code === 'number' ? result.code : undefined,
      result.stdout ?? '', typeof result.code === 'string' ? result.code : undefined)
  }
}
