...
- If `--dry-run` or `--preview` is present: set `DRY_RUN=true`. This flag is forwarded to every `/implement` call.
...
Execute waves sequentially. Within each wave, invoke `/implement` for all features in parallel (up to `CONCURRENCY` at a time).
...
- `running` — `/implement` invocation is active
- `done` — `/implement` completed successfully
- `failed` — `/implement` exited with an error
...
2. For each feature batch of size ≤ `CONCURRENCY` within the wave:
   - Invoke `/implement` with the feature refs and forwarded flags:
     ```
     /implement <ref1> <ref2> ... [--dry-run]
     ```
