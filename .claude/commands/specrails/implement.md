...
**MANDATORY: Always follow this pipeline exactly as written. NEVER skip, shortcut, or "optimize away" any phase — even if the task seems simple enough to do directly. The orchestrator MUST launch the sr-architect, sr-developer, and sr-reviewer agents as specified. Do NOT implement changes yourself in the main conversation; delegate to the agents defined in each phase. No exceptions.**
...
All tasks currently route to the full-stack **sr-developer** agent since the project doesn't have separate backend/frontend layers yet.
...
If a sr-test-writer agent fails or times out:
...
If a sr-doc-sync agent fails or times out:
...
If `FRONTEND_FILES` is empty: set `FRONTEND_REVIEW_REPORT = "SKIPPED"` and skip sr-frontend-reviewer launch. Note: "No frontend files detected."
If `BACKEND_FILES` is empty: set `BACKEND_REVIEW_REPORT = "SKIPPED"` and skip sr-backend-reviewer launch. Note: "No backend files detected."
...
5. Create PR (if GitHub CLI is available):
   ...
   ---
   Implemented via `/specrails:implement` pipeline (sr-architect → sr-developer → sr-reviewer)
   EOF
   )"
   ```
