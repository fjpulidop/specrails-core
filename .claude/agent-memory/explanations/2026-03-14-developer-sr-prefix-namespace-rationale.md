...
All specrails agents and workflow commands were renamed to use the `sr-` prefix (e.g., `architect` → `sr-architect`, `/implement` → `/specrails:implement`) and moved into a `sr/` command subdirectory.
...
The `sr/` subdirectory for commands (`.claude/commands/specrails/`) is what gives Claude Code the `/specrails:implement` slash syntax — the namespace comes from the directory name, not the file name.
