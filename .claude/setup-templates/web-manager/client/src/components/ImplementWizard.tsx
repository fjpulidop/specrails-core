...
export function ImplementWizard({ open, onClose }: ImplementWizardProps) {
  ...
  async function handleSubmit() {
    ...
    if (state.path === 'from-issues') {
      ...
      command = `/sr:implement #${state.selectedIssues[0]}`
    } else {
...
function PathCard({ icon, title, description, onClick }: PathCardProps) {
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        'flex flex-col gap-2 p-4 rounded-lg border border-border text-left',
        'hover:border-blue-500/40 hover:bg-blue-500/5 transition-all active:scale-[0.98]'
      )}
    >
