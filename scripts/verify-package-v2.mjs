...
export function verifyEngineV2({ root, installed, prefix, temp, env, contract }) {
  ...
  for (const entry of [bin, dist]) {
    ...
    assert.deepEqual(status.last.workflow, { id: published.id, version, source: 'definition', engine: 2, definitionHash: version })
  ...
  assert.equal(forkStatus.last.state.status, 'running')
  assert.equal(forkStatus.last.state.lease, null)
  assert.deepEqual(forkStatus.last.state.pendingInterrupts, [])
  const forkPaused = cliJson(bin, ['resume', '--context', forkContext], opts)
  assert.equal(forkPaused.code, 2, describe('fork reaches question', forkPaused))
  assert.equal(forkPaused.last.pendingQuestion?.stepId, 'ask')
