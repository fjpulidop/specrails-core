...
The programmatic runtime's **compact agent loop** (the default for OpenAI-compatible local engines) drives small models through host-owned structured steps and enforces process guardrails the models themselves get wrong: a repository's own test command runs after every task group, a test file no verification command reaches is a failure rather than a pass, a harness that prints failures but exits 0 is routed to the fixer, whole-file rewrite loops and binary-asset writes are refused, and the reviewer re-reviews only the files a fix round changed. Every guardrail is named in the catalog and can be switched off per project; CLI providers are untouched. See [agent runtime](docs/agent-runtime.md).
