...
/sr:setup                  # run the 5-phase wizard (~5 min)
```
...
| **Project data** | Generated by `/sr:setup` into `.specrails/` | Same |
| **Use case** | Recommended for most projects | When you need full offline control |

The plugin contains the logic (agent prompts, skills, commands). Running `/sr:setup` generates the project-specific data files (~8–10 files in `.specrails/`) — config, personas, memory, pipeline state. These are yours to edit and commit.
...
| **Commands** | Plugin | 17 workflow commands: `/sr:implement`, `/sr:product-backlog`, `/sr:why`, and more |
| **Skills** | Plugin | OpenSpec skills (`/opsx:*`) included |
| **Hooks & References** | Plugin | Agent hooks, reference docs |

### Project data (generated by `/sr:setup` — lives in your repo)
...
**Local tickets are the default.** The `/sr:setup` wizard defaults to local tickets and skips GitHub/JIRA credential setup unless you opt in.
...
**Can I re-run setup?**
Run `/sr:setup` again at any time to regenerate or update project data files.
