...
Auto-detect project dependencies: try `npm install`, `pip install -r requirements.txt`, `bundle install`, `go mod download`, `cargo build`, etc. based on what manifest files exist (package.json, requirements.txt, Gemfile, go.mod, Cargo.toml). Run whichever applies.

#### 4. Test runner

Auto-detect test runner: check for jest/vitest/mocha (package.json), pytest (requirements.txt/pyproject.toml), rspec (Gemfile), go test (go.mod), cargo test (Cargo.toml). Set TEST_CMD accordingly.
...
- Tag each task with its layer (e.g., frontend, backend, api, data, infra) as determined from CLAUDE.md
...
Route tasks based on file extensions and patterns: frontend files (*.tsx, *.css, components/) → sr:frontend-developer; backend files (*.rb, *.go, *.py, api/, db/) → sr:backend-developer; full-stack or mixed → sr:developer (default). Adjust routing based on stack described in CLAUDE.md.
...
5. Create PR (if GitHub CLI is available):
   ```bash
   gh pr create --title "<feat: descriptive title>" --body "<PR body with summary, test plan, and link to issues>"
   ```
