...
Check whether persona files exist at `{{SPECRAILS_DIR}}/agents/personas/`. This path is present in any repo that has run `{{COMMAND_PREFIX}}setup`.

```bash
ls {{SPECRAILS_DIR}}/agents/personas/ 2>/dev/null
```
...
---
_Generated by `{{COMMAND_PREFIX}}refactor-recommender` in {{PROJECT_NAME}}_
```
...
Where:
- `{vpc_header}` is `VPC personas loaded: {persona names}` when `VPC_AVAILABLE=true`, or `VPC personas: not found (run {{COMMAND_PREFIX}}setup to enable)` otherwise.
