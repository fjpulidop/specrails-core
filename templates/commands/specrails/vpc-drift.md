...
1. `{{SPECRAILS_DIR}}/agents/` — look for `.md` files whose content includes a `## Value Proposition Canvas` section.
...
Expected location: {{SPECRAILS_DIR}}/agents/*.md or {{PERSONA_DIR}}/*.md
Each persona file must contain a ## Value Proposition Canvas section.
Run {{COMMAND_PREFIX}}setup to generate persona files from templates.
```
...
1. **Cache:** Check whether `{{SPECRAILS_DIR}}/backlog-cache.json` exists and is valid JSON. If so, read all issues from it (`issues` map). Set `BACKLOG_SOURCE="cache"`.
...
Check whether `{{SPECRAILS_DIR}}/agent-memory/` exists. If it does, glob all `.md` files within it. For each file:
...
⚠️  <N> backlog items (<N>%) have no clear persona linkage.
    Consider running {{COMMAND_PREFIX}}auto-propose-backlog-specs to re-evaluate them.
```
...
2. Run `{{COMMAND_PREFIX}}auto-propose-backlog-specs` after updating personas to regenerate aligned feature ideas.
3. Re-run `{{COMMAND_PREFIX}}vpc-drift` after one sprint to measure improvement.

_Generated by `{{COMMAND_PREFIX}}vpc-drift` in {{PROJECT_NAME}} on <ISO date>_
```
...
After rendering, write a drift snapshot to `{{SPECRAILS_DIR}}/health-history/`:

1. Filename: `vpc-drift-<YYYY-MM-DD>.json`
2. Directory: `{{SPECRAILS_DIR}}/vpc-drift-history/` (create if absent, idempotent).
3. Content: the same JSON object described in Phase 5 (regardless of FORMAT setting).

Print: `Snapshot saved: {{SPECRAILS_DIR}}/vpc-drift-history/vpc-drift-<YYYY-MM-DD>.json`

If the write fails: print `Warning: could not write drift snapshot. Continuing.` Do not abort.

**Housekeeping:** If `{{SPECRAILS_DIR}}/vpc-drift-history/` has more than 30 `.json` files, print:
```
Note: {{SPECRAILS_DIR}}/vpc-drift-history/ has <N> snapshots. Prune old ones with:
  ls -t {{SPECRAILS_DIR}}/vpc-drift-history/ | tail -n +31 | xargs -I{} rm {{SPECRAILS_DIR}}/vpc-drift-history/{}
```
